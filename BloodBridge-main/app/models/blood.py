...
class BloodRequest(db.Model):
    ...
    allocations = db.relationship('BloodAllocation', backref='request', lazy=True)
...
class BloodAllocation(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    request_id = db.Column(db.Integer, db.ForeignKey('blood_request.id', name='fk_bloodallocation_request_id'), nullable=False)
    inventory_id = db.Column(db.Integer, db.ForeignKey('blood_inventory.id', name='fk_bloodallocation_inventory_id'), nullable=False)
    units_allocated = db.Column(db.Integer, nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    inventory = db.relationship('BloodInventory', backref=db.backref('allocations', lazy=True))

    def __repr__(self):
        return f'<BloodAllocation {self.request_id} - {self.units_allocated} units>'
