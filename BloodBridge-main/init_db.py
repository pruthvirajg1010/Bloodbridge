...
#     init_db() 
from app import create_app, db
from app.models import User, BloodBank, Hospital, BloodDrive, BloodInventory, BloodRequest, Admin, Donor
from datetime import datetime, timedelta
from werkzeug.security import generate_password_hash
...
def init_db():
    ...
    with app.app_context():
        ...
        inventory_items = [
            BloodInventory(blood_bank_id=blood_bank.id, blood_type='O+', units_available=100),
            BloodInventory(blood_bank_id=blood_bank.id, blood_type='A-', units_available=50),
            BloodInventory(blood_bank_id=blood_bank.id, blood_type='B+', units_available=75),
            BloodInventory(blood_bank_id=blood_bank.id, blood_type='AB+', units_available=20),
        ]
        ...
        db.session.add_all(
            inventory_items + [blood_drive1, blood_drive2, blood_request1, blood_request2]
        )
